...
async def probe_status_api(client: httpx.AsyncClient, base_url: str) -> tuple[list[ProbeResult], dict[str, Any] | None]:
    ...
    try:
        ...
        if r.status_code != 200:
            return _all_unknown(elapsed_ms, f"HTTP {r.status_code}"), None
        body = r.json()
    except httpx.TimeoutException:
        return _all_unknown(int((time.perf_counter() - started) * 1000), "timeout"), None
    except (httpx.HTTPError, ValueError) as exc:
        return _all_unknown(int((time.perf_counter() - started) * 1000), str(exc)[:200]), None
...
def _all_unknown(elapsed_ms: int, error: str) -> list[ProbeResult]:
