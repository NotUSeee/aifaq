...
async def probe_discord(client: httpx.AsyncClient, bot_token: str) -> ProbeResult | None:
    """Hit Discord's /users/@me with the bot token to confirm Discord
    can authenticate the bot. Returns None if no token is configured.
    `bot_token` should be a token with `identify` scope only."""
    if not bot_token:
        return None
